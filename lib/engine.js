...
export default class Engine {
  ...
  drawHUD() {
    ...
    this.actions.forEach(action => {
      ...
      actionEl.textContent = this.actionTexts[action.id] || action.id.replace("-", " ");
  ...
  drawObjects() {
    ...
    Object.keys(this.state.objects).forEach(objectId => {
      ...
      const spanEl = document.createElement("span");
       spanEl.textContent = this.itemNames[objectId] || objectId;
        ...
        entityEl.addEventListener("pointerenter", e => {
          ...
          if (actionEl) {
            ...
            if(entity.tooltip) {
              ...
              text = this.actionTexts[actionEl.dataset.action] || actionEl.dataset.action.replace("-", " ");
              if (text === "使用" && objectEl) {
                text += " " + (this.itemNames[objectEl.dataset.object] || objectEl.dataset.object) + " 对着";
              }
              text += " " + (this.itemNames[entity.id] || entity.id);
            }
